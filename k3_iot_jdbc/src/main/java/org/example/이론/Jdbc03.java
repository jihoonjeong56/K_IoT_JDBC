...
/*
    ...
    1) SQL 작성 방법
        - Statement : SQL 직접 작성
        - PrepareStatement : ?를 사용해 파라미터 처리
    2) 보안
        - Statement : SQL Injection 위험 있음
        - PrepareStatement : SQL Injection 방지
        
        cf) SQL Injection : 악의적인 SQL문 실행(SQL 쿼리를 일반 입력 또는 약식 필드에 삽입하여 보안 취약성 공격)
    3) 성능
        - Statement : 매번 컴파일
    ...
    2) exㄷcuteUpdate
    : INSERT / UPDATE / DELETE 문 실행시 사용 - 실행 결과로 영향 팓은 레코드 수를 반환
    - 반환 타입 INT
    - 행의 개수를 반호나하기 때문에 rs(ResultSet) 사용 필요 없음
