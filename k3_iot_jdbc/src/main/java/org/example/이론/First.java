...
public class First {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 2. DB 연결
            // : 실제 MySQL  DB 에 접속 시도(성공 시 Connection 객체를 반환)
...
/*
            ...
            pstmt = conn.prepareStatement(sql); // 동적 데이터 값은 Set 설정 하수로 설정
            pstmt.setString(1,"이도경");
            pstmt.setString(2,"qwesdf33@example.com");

            int result = pstmt.executeUpdate(); // INSERT 실행 - 반환은 연향받은 행의 수
...
*/
           ...
           /*
            ...
            while (rs.next()) { // rs.next() : 다음 행 조재 여부 확인 boolean 값 반환
                int id = rs.getInt("id");
            ...
            */
            ...
            System.out.println("DB 와 의 커넥션 중 예외발생");
            e.printStackTrace(); // - 예외 발생시 예외 메시지 + 호출 스택 정보 출력(예외 발생지 출력)
        } finally {
            // 자원 해제( 자바7버전 이상 - 자동 자원 해제) >> 생략 가능
            try {
